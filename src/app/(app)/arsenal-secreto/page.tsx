...
export default function ArsenalSecretoPage() {
  ...
  const launched = product.status === "live" && product.checkoutUrl;
    ...
    <div className="relative isolate min-h-dvh overflow-hidden">
      ...
      <div className="mx-auto w-full max-w-5xl px-6 pt-16 pb-24 md:px-10">
        ...
        <div className="animate-rise">
          <SitePreview
            url={product.salesUrl}
            previewImage={previewImage}
            cta={
              launched
                ? { label: "Quero entrar", href: product.checkoutUrl! }
                : { label: "Em breve", href: product.salesUrl }
            }
          />
        </div>

        <p className="mt-10 text-center text-sm text-muted">
          {launched ? product.tagline : "O cofre está sendo preparado."}
        </p>
